...
export const Route = createFileRoute("/tv")({
  head: () => ({ meta: [{ title: "carrossel · casamento d & l" }] }),
  component: TVPage,
});
...
function TVPage() {
  ...
  const load = async () => {
    const { data } = await supabase
      ...
      .eq("visibility", "public").eq("hidden", false).eq("type", "image")
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center text-white relative" style={{ backgroundColor: "var(--bg-dark)" }}>
      ...
      {!cur ? (
        ...
        <div key={cur.id} className="text-center px-6 fade-in">
          <div className="flex justify-center mb-6">
            <SignedImage path={cur.storage_path} driveFileId={cur.drive_file_id} driveThumbnailUrl={cur.drive_thumbnail_url} driveViewUrl={cur.drive_view_url} type="image" full className="max-h-[60vh] rounded-2xl shadow-elegant" />
          </div>
      ...
      <p className="absolute bottom-24 text-[11px] text-white/40 tracking-wide">só fotos públicas · privadas nunca aparecem aqui</p>
    </div>
  );
}
